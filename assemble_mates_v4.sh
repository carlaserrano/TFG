...
QIIME_DIR=/usr/lib/qiime/bin	# if Ubuntu-installed
QIIME_DIR=~/contrib/QIIME/
export PATH=$QIIME_DIR/bin:$PATH

maxdepth=1000000

nproc=`nproc`
nproc=$((nproc / 2))
echo "$0 will run using $nproc processors if possible"
...
function bwa_reference_assemble() {
	...
	echo "Aligning R1"
        bwa aln -t $nproc reference.fna R1.fastq > s1.bwa.sai
        # we may also try this
        #bwa mem -t 8 reference.fna R2.fastq > s2.bwa.sai
    fi
    if [ ! -e s2.bwa.sai ] ; then
	echo "Aligning R2"
        bwa aln -t $nproc reference.fna R2.fastq > s2.bwa.sai
        # we may also try this
        #bwa mem -t 8 reference.fna R2.fastq > s2.bwa.sai
    fi
    if [ ! -e aligned.bwa.sam ] ; then
    READ_GRP=$base
    RG='@RG\tID:1\tPL:ILLUMINA\tSM:'$READ_GRP'\tDS:ref='reference.fna',pfx='$base
	...
	echo "Aligning mate pairs"
        bwa sampe -P reference.fna \
            ...
            R1.fastq R2.fastq \
            | tee aligned.bwa.sam \
            | samtools view -b -S - > aligned.bwa.bam;
    fi
...
function bowtie2_reference_assemble() {
    ...
    if [ ! -e aligned.bowtie.sam ] ; then
        ...
        bowtie2 -p $nproc --qc-filter -x reference \
...
function analyze_sam_file() {
    ...
    # Postprocess to indexed sorted BAM
    if [ ! -s ${base}.bam ] ; then
        echo "Converting ${base}.sam to ${base}.bam"
        samtools view -b ${base}.sam -o ${base}.bam
    fi
    if [ ! -s ${base}.sorted.bam ] ; then
        echo "Sorting ${base}.bam"
        samtools sort --threads $nproc ${base}.bam -o ${base}.sorted
    fi
    if [ ! -s ${base}.sorted.bam.bai ] ; then
	echo "Creating ${base}.sorted.bam.bai"
        samtools index ${base}.sorted.bam
    fi
    if [ ! -s ${base}.flagstat.txt ] ; then
        echo "---------------------------------------------------------";
        echo "Collecting alignment statistics";
        echo "---------------------------------------------------------";
        echo "Running flagstat...";
        samtools flagstat ${base}.sorted.bam | tee ${base}.flagstat.txt
    fi
    ...
    if [ ! -s ${base}.variants.vcf ] ; then
	echo "Doing variant calling"
        samtools mpileup -d $maxdepth -g -f $ref ${base}.sorted.bam \
 	    > ${base}.variants.bcf
        #bcftools view -c -v ${base}.variants.bcf > ${base}.variants.vcf
        bcftools view  -v "snps,indels,mnps,ref,bnd,other" \
            ${base}.bwa.variants.bcf \
            > ${base}.bwa.variants.vcf
    fi

    vcfutils=`which vcfutils.pl`
    if [ ! -x $vcfutils ] ; then
        # old versions had it installed here
	...
	#samtools mpileup -d $maxdepth -v -f $ref ${base}.sorted.bam > E.vcf 
	#bcftools consensus -f $ref E.vcf -o E.c.vcf
	#cat E.c.vcf | vcfutils.pl vcf2fq > E.fq
    fi
    
    if [ -s ${base}.cons.fastq -a ! -s ${base}.cons.fna ] ; then
	echo "analyze_sam_file: making fasta from ${base}.cons.fastq"
        fq2f ${base}.cons.fastq > ${base}.cons.fna
    else
        echo "analyze_sam_file: WARNING! Consensus fastq file size is zero"
    fi

    # Compute coverage
    if [ ! -e ${base}.cov.stats ] ; then
	echo "Computing coverage"
        samtools mpileup -d $maxdepth -ABR ${base}.sorted.bam > ${base}.coverage
...
function a5_denovo_assemble() {
	...
	export PATH=~/contrib/a5/bin:$PATH
        a5_pipeline.pl --trheads=$nproc R1.fastq R2.fastq genome
    fi
    cd ..
}
...
function usage()
{
    ...
    echo ""
    echo "    -W --bwa         Align mate pairs using BWA"
    ...
    echo "    -o outdir        The directory where output should be saved"
    echo "    -1 forward.fq    A file with the forward reads in FastQ format"
    echo "    -2 reverse.fq    A file with the reverse reads in FastQ format"
    echo ""
    ...
    echo "    or be specified with -1 and -2"
    echo "    They can be optionally compressed with gzip"
    echo ""
    echo "Output will be saved in a directory named \$out"
    echo "  the assemblies will be in subdirectory \$seqdir_x_\$ref"
    echo ""
    return
}
...
TEMP=`getopt -o ho:vriWBLMAmlnS:1:2:R: \
     --long help,out,verbose,roche454,illumina,bwa,bowtie2,lastass,maq,a5,mummer3,last,nucmer,sequences:,R1:,R2:,reference: \
     -n "$0" -- "$@"`
...
# Set up reads ID
READS="orig"		# in case we got no SEQDIR

# Verify existence of and set up reads files (R1 and R2)
if [ ! -d "$SEQDIR" ] ; then
...
# work in chosen output directory
if [ ! -d "$OUT" ] ; then
    if [ -e "$OUT" ] ; then
        echo "ERROR: $OUT exists and is not a directory"
        exit
    fi
    mkdir -p "$OUT"
fi
...
if [ "$SEQUENCE_IS_454" = "yes" ] ; then
    ...
    if [ ! -e reference.fna ] ; then
        # this should always work, we have already checked
        if [ -e "$REFERENCE" ] ; then
