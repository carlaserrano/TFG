...
for (pfu in c("0.1", "001", "010", "100")) {
    ...
    for (dir in c(1, 2)) {
        ...
        rd.start <- subset(dist.seg.A, SIZ < 0)$LEFT
        rd.end <- subset(dist.seg.A, SIZ < 0)$RIGHT
        
        se.hist <- paste(basename, '.se.o.', int.len, '.A.png', sep='') 
        ...
        gg.hist <- paste(basename, ".ES.O.", int.len, '.A.png', sep='')
        ...
        rd.start <- subset(dist.seg.B, SIZ <0)$LEFT
        rd.end <- subset(dist.seg.B, SIZ <0)$RIGHT
        se.hist <- paste(basename, '.se.o.', int.len, '.B.png', sep='') 
        png(se.hist, width=1000, height=1000)
          hist( rd.start, 
              breaks = calc.intervals(rbind(rd.start, rd.end), int.len),
              border = "blue", 
        ...
        dev.off()
        gg.hist <- paste(basename, ".ES.O.", int.len, '.B.png', sep='')
        print(paste("        ggplotting to", gg.hist))
        #png(gg.hist, width=1000, height=1000)
          es <- data.frame(Start = rd.start, End = rd.end)
          ggplot(melt(es), 
    ...
    dist.seg.A <- subset(dist, SEG == "A" & SIZ < 0)
    dist.seg.B <- subset(dist, SEG == "B" & SIZ < 0)
    rd.start <- dist$LEFT
    rd.end <- dist$RIGHT
    
    # Plots for segment A
    png(paste(basename, ".sizes.0.A.png", sep=''), width=1000, height=1000)
    with(dist.seg.A, 
         Hist(-SIZ, groups=READ, scale="percent", breaks=75, 
	      col="purple", main=paste(pfu, "pfu: size A"))
	 )
    dev.off()
    png(paste(basename, ".left.o.A.png", sep=''), width=1000, height=1000)
    with(dist.seg.A, 
         Hist(LEFT, groups=READ, scale="percent", breaks=75, 
	      col="purple", main=paste(pfu, "pfu: left A"))
	 )
    dev.off()
    png(paste(basename, ".right.o.A.png", sep=''), width=1000, height=1000)
    with(dist.seg.A, 
         Hist(RIGHT, groups=READ, scale="percent", breaks=75, 
	      col="purple", main=paste(pfu, "pfu: right A"))
	 )
    dev.off()

    # Plots for segment B
    png(paste(basename, ".sizes.o.B.png", sep=''), width=1000, height=1000)
    with(dist.seg.B, 
         Hist(-SIZ, groups=READ, scale="percent", breaks=75, 
	      col="purple", main=paste(pfu, "pfu: size B"))
	 )
    dev.off()
    png(paste(basename, ".left.o.B.png", sep=''), width=1000, height=1000)
    with(dist.seg.B, 
         Hist(LEFT, groups=READ, scale="percent", breaks=75, 
	      col="purple", main=paste(pfu, "pfu: left B"))
	 )
    dev.off()
    png(paste(basename, ".right.o.B.png", sep=''), width=1000, height=1000)
    with(dist.seg.B, 
         Hist(RIGHT, groups=READ, scale="percent", breaks=75, 
	      col="purple", main=paste(pfu, "pfu: right B"))
	 )
    dev.off()
}
